...
class Funciones {
	...
	public function ListAsistentes($NROPROG){
		...
		WHERE CP.NROPRGC = '$NROPROG' AND CP.REPROBADO = 2";
	...
	public function ListCapCompletas($Filtrado){
		...
		$Da="SELECT TP1.OPCION NCategoria, TP2.OPCION NSubtipo, P.Anio, P.Mes, P.NROPROG, IIF(P.TFORM = 1, 'Interna', 'Externa') AS TFORM, IIF(P.CumpLegal = 1, 'Sí', 'No') AS Cumpleg,
		P.CAPACITACION, P.PRECIO, P.CANTIDADASIS, P.CAPACITADOR
		FROM PLATCAPACITACIONES.dbo.Programacion P
		INNER JOIN PLATCAPACITACIONES.dbo.Capacitaciones C ON C.NROPRGC = P.NROPROG
		INNER JOIN PLATCAPACITACIONES.dbo.CalCapacitador L ON L.NROPROG = P.NROPROG
		...
		LEFT JOIN PLATCAPACITACIONES..OpcionesProgramacion TP2 ON TP2.ID = P.SUBTIPO
		WHERE M.CODIGO = '$Filtrado' 
	...
	public function ListCapCompletasTrim($Anio, $Trimestre, $CODIGO, $Categoria, $Subtipo, $Sexo, $COSTOCC){
		...
		$Da="SELECT TP1.OPCION NCategoria, TP2.OPCION NSubtipo, P.NROPROG, P.ANIO, P.MES, IIF(TFORM = 1, 'Interna', 'Externa') AS TFORM, 
			...
			P.CAPACITADOR, P.CANTIDADPROG,P.CANTIDADASIS, P.PRECIO
		FROM PLATCAPACITACIONES.dbo.Programacion P 
		...
		GROUP BY P.NROPROG, P.ANIO, P.MES, P.TFORM, P.CumpLegal, P.CAPACITACION, TP1.OPCION, TP2.OPCION, P.CAPACITADOR, P.CANTIDADPROG, P.CANTIDADASIS, P.PRECIO, TP1.OPCION, TP2.OPCION";
	...
	public function CabCap($NROPROG){
		...
		FROM PLATCAPACITACIONES.dbo.Programacion P 
		INNER JOIN PLATCAPACITACIONES.dbo.CalCapacitador C ON C.NROPROG = P.NROPROG
	...
	public function PartiCapac($NROPROG , $CODEMPLEADO){	
		...
		$Da="SELECT CONCAT(RTRIM(M.NOMBRE), ' ', RTRIM(M.APELLIDO)) AS NOMBRE, CONCAT(RTRIM(M.NOMBRE), ' ', RTRIM(M.NOMBRE2), ' ', RTRIM(M.APELLIDO), ' ', RTRIM(M.APELLIDO2)) AS NOM, M.CARGO, M.CEDULA,M.CODIGO, C.APRUEBA, C.Observaciones FROM PLATCAPACITACIONES.dbo.Programacion P INNER JOIN PLATCAPACITACIONES.dbo.Capacitaciones C ON C.NROPRGC = P.NROPROG INNER JOIN PLATCAPACITACIONES.dbo.CalCapacitador L ON L.NROPROG = P.NROPROG INNER JOIN PALMERAS2013.dbo.MTEMPLEA M ON M.CODIGO = C.CODIGOEMPL WHERE P.NROPROG = $NROPROG $Codempl";
		$Dat = odbc_exec($conexion, $Da);
		while ($Dos = odbc_fetch_array($Dat)) { $arr[] = $Dos; }
		return $arr;				
	}
	...
	public function generarExcelProgramaciones($fecha1,$fecha2){	
		...
		$Da="SELECT P.NROPROG,P.FECHA,P.CANTIDADPROG,P.CANTIDADASIS,p.CAPACITACION,P.CAPACITADOR,P.ANIO,P.MES,
			...
			IIF(TFORM = 1, 'Interna', 'Externa') AS TFORM, IIF(P.CumpLegal = 1, 'Sí', 'No') AS Cumpleg,
			...
			WHERE P.FECHA BETWEEN '$fecha1-01' AND '$fecha2-01' ORDER BY P.NROPROG DESC";
		$Dat = odbc_exec($conexion, $Da);
		while ($Dos = odbc_fetch_array($Dat)) { $arr[] = $Dos; }
		return $arr;				
	}
